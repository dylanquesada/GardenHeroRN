...
export async function addPlantToFirebase(name, date) {
  ...
  db
    .ref("users/" + userID + "/garden/plants/" + numberOfPlants.toString())
    .set({
      name: name,
      plantDate: date
    });
...
export async function populateTasksForAdd(garden) {
  ...
  for (
    let i = plantDate;
    i <= plantDate + master.tomato.lifetime * oneDay;
    i += master.tomato.daysBetweenWatering[waterCounter] * oneDay
  ) {
  ...
  for (
    let i = plantDate + master.tomato.daysUntilHarvest * oneDay;
    i <= plantDate + master.tomato.lifetime * oneDay;
    i += master.tomato.harvestInterval * oneDay
  ) {
